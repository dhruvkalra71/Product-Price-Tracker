...
async def scrape_product_async(
    ...
    t_start = time.time()
    log(f"Starting scrape for product_id={source_product_id} (headed={headed})")
    ...
    async with async_playwright() as p:
        ...
        # Try msedge first on Windows if chromium binary is not cached, otherwise default chromium
        if sys.platform == "win32":
            try:
                browser = await p.chromium.launch(channel="msedge", **launch_options)
                log("Launched Microsoft Edge (Chromium engine)")
            except Exception:
                pass
        ...
        if browser is None:
            try:
                ...
                log("Launched Playwright Chromium")
